...
namespace App
{
  ...
  template <int dim>
  HelmholtzProblem<dim>::AssemblyScratchData::
  AssemblyScratchData (const FiniteElement<dim> &fe)
    :
    fe_values (fe,
               QGauss<dim>(2),
  ...
  void HelmholtzProblem<dim>::assemble_system ()
  {
    WorkStream::run(dof_handler.begin_active(),
                    dof_handler.end(),
                    *this,
                    &HelmholtzProblem::local_assemble_system,
                    &HelmholtzProblem::copy_local_to_global,
  ...
  template <int dim>
  void
  HelmholtzProblem<dim>::copy_local_to_global (const AssemblyCopyData &copy_data)
  {
          ...
          for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point)
            {
              const double neumann_value
                = (exact_solution.gradient (scratch_data.fe_face_values.quadrature_point(q_point)) *
                   scratch_data.fe_face_values.normal_vector(q_point));

              for (unsigned int i=0; i<dofs_per_cell; ++i)
                copy_data.cell_rhs(i) += (neumann_value *
...
int main (int argc, char *argv[])
{
  ...
  if (argc<2) {
    std::cout << "Usage: ./step-7 number_of_steps" << std::endl;
    exit(1);
  }
